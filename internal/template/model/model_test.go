...
func TestIgnListJson_MarshalUnmarshal(t *testing.T) {
	original := IgnListJson{
		Version:     "1.0",
	...
	if len(decoded.Templates) != len(original.Templates) {
		t.Fatalf("Templates length: expected %d, got %d", len(original.Templates), len(decoded.Templates))
	}

	// Verify first template
	first := decoded.Templates[0]
	if first.Name != "go-basic" {
		t.Errorf("Templates[0].Name: expected 'go-basic', got %s", first.Name)
	}
	if first.Category != "backend" {
		t.Errorf("Templates[0].Category: expected 'backend', got %s", first.Category)
	}
	if len(first.Tags) != 3 {
		t.Errorf("Templates[0].Tags length: expected 3, got %d", len(first.Tags))
	}
}
