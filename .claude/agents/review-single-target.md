...
4. **Post review comments to GitHub PR** ⚠️ **MANDATORY STEP - DO NOT SKIP**:
...
```markdown
## 🔍 Critical: Missing error handling in user creation
...
func CreateUser(data UserData) User {
    user, _ := db.Insert(data)  // ❌ Ignoring error
    return user
}
```
...
Status: ✅ Already Fixed | ❌ Not Fixed | ⚠️ Partially Addressed
...
- ✅ DO: Describe the problem and why it's problematic
- ✅ DO: Show brief code snippet (2-5 lines) illustrating the issue
- ✅ DO: Provide high-level direction for fixes
- ❌ DON'T: Write complete fix implementations
- ❌ DON'T: Provide detailed refactored code
- ❌ DON'T: Show extensive code examples
...
⚠️ **NOT Review Targets** (automatically checked by commands - skip these):
