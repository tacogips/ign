...
## Issue 49 Selective Overwrite Manifest Pruning

**Status**: Completed

## Spec Reference

- `design-docs/specs/selective-overwrite.md#removed-managed-file-cleanup`
- Issue: `tacogips/ign#49`

## Implemented

- [x] Pruned absent stale manifest entries during selective overwrite even when
  their generated paths match the template `.ign-overwrite-ignore`
  (`internal/app/update_cleanup.go`, `internal/app/manifest.go`).
- [x] Preserved existing ignored stale paths on disk and retained their prior
  manifest entries (`internal/app/update_cleanup.go`).
- [x] Reported absent stale manifest pruning through the same `D`
  classification used by dry-run, confirmation preview, and write-path update
  results (`internal/app/update_cleanup.go`, `internal/app/update.go`,
  `internal/cli/update.go`).
- [x] Added focused app regressions for absent ignored stale pruning, no
  ignored-path recreation, existing ignored stale preservation, and missing
  stale dry-run reporting (`internal/app/update_test.go`).
...
- Issue #49 is a manifest migration and reconciliation fix. It does not change
  ignored-path creation filtering from issue #48.
- Existing issue #45 directory-to-symlink transition exclusions remain in the
  cleanup path and continue to prevent cleanup from following replacement
  symlinks.
- `--overwrite-all` and `--force` keep overwrite-all cleanup semantics and do
  not consult `.ign-overwrite-ignore`.

## Notes

- Verification passed with focused issue #49 app tests, full app/template/CLI
  package tests, full Go tests, build, vet, and diff hygiene.
...
- Already-missing stale managed paths are pruned from `.ign/ign-files.json` and
  reported with `D` as manifest-prune removals.
...
- Already-missing stale manifest entries are pruned and reported with `D` so
  previews and write-path summaries share one removed-managed-path
  classification.
