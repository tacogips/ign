...
| `ign-var.json` | `.ign-build/` | User variables and template reference | Yes (for init) |
| `ign-list.json` | User-defined | Collection of template references | No |
...
**Location:** `.ign-build/ign-var.json` (or custom path via `--config`)
...
- Prefix: `@file:`
- Path: Relative to `.ign-build/` directory
- Example: `@file:license.txt` resolves to `.ign-build/license.txt`
...
Creates `.ign-build/ign-var.json` with empty variable values:
...
## 3. Template List (`ign-list.json`)

### 3.1 Purpose

Collection of template references for managing multiple templates.

**Location:** User-defined (e.g., `~/.config/ign/templates.json` or project-specific)

**Optional:** Not required for basic usage

**Use Cases:**
- Maintain catalog of organization templates
- Quick access to frequently used templates
- Share template collections with team
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `version` | string | Yes | List format version (currently "1.0") |
| `name` | string | No | List name |
| `description` | string | No | List description |
| `templates` | array | Yes | Array of template definitions |
...
| `path` | string | No | Subdirectory in repository |
| `ref` | string | No | Default git ref (branch/tag) |
| `description` | string | No | Template description |
| `tags` | string[] | No | Searchable tags |
| `category` | string | No | Template category |
| `maintainer` | string | No | Maintainer name/email |
...
```json
{
  ...
  "description": "Standard project templates for MyCompany",
  ...
  "templates": [
    {
      ...
      "path": "go/basic",
      "ref": "v1.0.0",
      "description": "Basic Go application with CLI",
      "tags": ["go", "cli", "basic"],
      "category": "backend",
    ...
    },
    {
      ...
      "path": "go/rest-api",
      "ref": "v2.1.0",
      "description": "Production-ready Go REST API with PostgreSQL",
      "tags": ["go", "api", "rest", "postgresql"],
      "category": "backend",
      "maintainer": "platform-team@mycompany.com"
    },
    {
      ...
      "ref": "v1.5.0",
      "description": "Go gRPC microservice template",
    ...
    },
    {
      ...
      "category": "frontend",
      "maintainer": "frontend-team@mycompany.com"
    }
  ]
}
```

### 3.5 Usage (Future Feature)

```bash
# Initialize from list
ign build init --from-list ./ign-list.json --template go-rest-api

# List available templates
ign list --file ./ign-list.json

# Search templates by tag
ign list --file ./ign-list.json --tag api

# Show template details
ign show go-rest-api --file ./ign-list.json
```

---

## 4. Global Configuration (`config.json`)
...
```json
{
  ...
  "defaults": {
    "build_dir": ".ign-build",
    "list_file": "~/.config/ign/templates.json"
  }
}
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `build_dir` | string | `".ign-build"` | Default build directory name |
| `list_file` | string | `""` | Default template list file path |
...
```json
{
  ...
  "defaults": {
    "build_dir": ".ign-build",
    "list_file": "~/.config/ign/company-templates.json",
    "output_dir": "."
  }
}
```

### 4.5 Priority and Overrides
...
```
my-workspace/
├── .ign-build/               # Build config (untouched)
│   ├── ign-var.json
│   ├── license-header.txt
│   └── templates/
│       └── intro.md
...
### 5.4 Global Configuration Directory
...
└── templates.json            # Default template list (optional)
```
...
.git/                         # Git directory
.ign-build/                   # Build configuration
```

### 6.2 Template-Specific Ignores
...
### 7.3 Binary Detection Algorithm
...
**ign-list.json schema:** `https://ign-tool.org/schema/ign-list.json.schema.json`
...
### 8.3 Validation Command (Future Feature)

```bash
# Validate config files
ign validate --config .ign-build/ign-var.json
...
### ign-list.json Schema

```typescript
interface IgnListJson {
