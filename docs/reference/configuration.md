...
**Note:** The `hash` field is auto-generated by `ign template collect-vars` and should not be manually edited.
...
| `hash` | string | No | SHA256 hash of template files (auto-generated by `collect-vars`) |

**Note on `hash` field:** The `hash` field is automatically calculated and updated when running `ign template collect-vars`. It represents a SHA256 hash of all template files (excluding `ign.json` itself). This hash is used by `ign update` to detect template changes.
