...
| `ign-var.json` | `.ign/` | User variables and template reference | Yes (for init) |
...
**Location:** `.ign/ign-var.json` (or custom path via `--config`)
...
Creates `.ign/ign-var.json` with empty variable values:
